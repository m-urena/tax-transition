...
def compute_buys(holdings_df, sell_fracs, target_model_df, total_portfolio_value):
    ...
    return buy_df[['Ticker', 'Target Weight %', 'Buy Value']].reset_index(drop=True)
...
with st.sidebar:
    ...
    custom_pct = st.slider("Tax Budget (% of max)", 0, 100, 52, format="%d%%")
    ...
    if selected_model_name == "Upload custom":
        ...
        st.caption("Edit tickers and weights directly:")
        custom_model_raw = st.data_editor(
            DEFAULT_MODEL, num_rows="dynamic",
    ...
    if data_source == "Upload file":
        ...
        raw = st.data_editor(DEFAULT_HOLDINGS, num_rows="dynamic",
                             use_container_width=True, hide_index=True)
    else:
        raw = DEFAULT_HOLDINGS.copy()
...
with st.spinner("Computing efficient frontier..."):
    frontier = compute_frontier(holdings, MAX_TAX, target_model, TOTAL_VALUE)

scenario_defs = [
    dict(name='Minimum Tax',    budget=0.0,                          color=GREEN),
    dict(name='Max Transition', budget=MAX_TAX,                      color=RED),
    dict(name='Custom',         budget=MAX_TAX * custom_pct / 100.0, color=BLUE),
]
scenarios = []
...
m2.metric("Gross Unrealized Gain", f"${total_gains:,.0f}",
          delta=f"+{total_gains/TOTAL_VALUE*100:.1f}%" if TOTAL_VALUE else None)
m3.metric("Gross Unrealized Loss", f"${abs(total_losses):,.0f}",
          delta=f"{total_losses/TOTAL_VALUE*100:.1f}%" if TOTAL_VALUE else None,
          delta_color="inverse")
m4.metric("Max Tax Liability",     f"${MAX_TAX:,.0f}",
          delta=f"{MAX_TAX/TOTAL_VALUE*100:.1f}% of portfolio" if TOTAL_VALUE else None,
          delta_color="off")
m5.metric("Portfolio Overlap",     f"{overlap_pct:.1f}%",
          delta="already in target model", delta_color="off")
...
for col, sc in zip(cols, scenarios):
    ...
    with col:
        ...
        <div class="sc-body">
          ...
          <hr class="sc-divider"/>
          <div class="sc-label" style="margin-top:8px;">Target Alignment</div>
        </div>
        """, unsafe_allow_html=True)
        st.plotly_chart(make_donut(sc['transition_pct'], sc['color']),
...
st.caption("Each point on the curve is the maximum transition achievable for a given tax budget.")
if not frontier.empty:
    st.plotly_chart(make_frontier(frontier, scenarios),
                    use_container_width=True, config={'staticPlot': True})
else:
    st.info("Frontier could not be computed for this portfolio.")
    ...
    with tab:
        ...
        with buy_tab:
            ...
            if buy_df is not None and not buy_df.empty:
                st.dataframe(
                    buy_df.style
                    .format({'Target Weight %': '{:.2f}%', 'Buy Value': '${:,.0f}'}),
                    use_container_width=True, hide_index=True,
                )
                c1, c2 = st.columns(2)
